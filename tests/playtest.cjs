...
(async () => {
 ...
 const results=await page.evaluate(()=>{
  ...
  f.reset();enter();r(['ArrowUp','KeyQ'],.45);run(['KeyW'],s=>s.truck.y<328);r(['Space'],.7);run(['KeyW','KeyD'],s=>s.truck.a>-.03);r(['Space','KeyA'],.13);r(['Space'],.6);r(['ArrowUp'],.65);run(['KeyW'],s=>s.truck.x>605);r(['Space'],.8);r(['ArrowDown'],1.3);r([],1.5);ensure(!s().won&&!s().pallet.support&&s().pallet.z===0,'overhanging shelf placement fails without levitation');ensure(s().pallet.x<697,'low cargo stays outside shelf face');out.shelfEdge='pass';
